Hi there 👋
# Hi, I am Dev Luki 👋

Full Stack Developer focused on building clean, scalable, and user centered web applications.  
I enjoy solving real world problems through thoughtful design, efficient code, and continuous learning.

📍 Nairobi, Kenya  
💼 Open to remote opportunities and freelance projects

## 🛠 Tech Stack

**Frontend**
- HTML
- CSS
- JavaScript
- React

**Backend**
- Node.js
- Express.js
- 
**Database**
- MongoDB
- Superbase

**Tools & Platforms**
- Git & GitHub
- VS Code

## 📊 GitHub Stats

![Lukii's GitHub stats](https://github-readme-stats.vercel.app/api?username=lukii3564&show_icons=true&theme=tokyonight)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=lukii3564&layout=compact&theme=tokyonight)

## 🚀 Featured Projects

- **Portfolio Website**  
  A personal portfolio showcasing my skills, projects, and contact information.  
  Tech used: HTML, CSS, JavaScript  

- **React Frontend Practice**  
  Hands on practice mastering React components, JSX, and styling techniques.  
  Tech used: React, JavaScript  

- **Web Development Projects**  
  A collection of frontend and full stack experiments focused on layout, responsiveness, and UI logic.

  ## 📫 Connect With Me

- Email: lukoyejackson77@gmail.com  
- LinkedIn:https://www.linkedin.com/in/lukoye-jackson-b86a352a2/

I am always open to collaborations, learning opportunities, and meaningful conversations around technology.
